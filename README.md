...
솔도로스는 납도(納刀) 컨셉을 구현한 캐릭터입니다.
손패의 카드를 덱 맨 위로 되돌리는 비용을 치르는 대신, 다른 캐릭터 대비 높은 위력을 발휘하도록 설계되어 있습니다.
전략적인 손패 운용과 다양한 빌드 구성으로 탑을 정복해보세요.
...
### 🌊 류심 (Flowing Stance)
류심 카드를 사용하면 **류심 승 / 류심 쾌 / 류심 충** 3장의 토큰 공격 카드가 손으로 들어옵니다.  
...
### 🛡️ 방어도 파괴 (Block Break) 시너지
적의 방어도가 남아 있을수록 위력이 증가하는 카드와 유물이 존재합니다.  
단순히 적의 방어도를 깨는 것에 그치지 않고, **적에게 직접 방어도를 부여하는 카드**로  
조건을 능동적으로 만들어 시너지를 극대화하는 독특한 플레이가 가능합니다.
...
- 🇰🇷 한국어 (Korean)
- 🇺🇸 영어 (English)
- 🇨🇳 중국어 간체 (Simplified Chinese) — Special Thanks to ChengZi47
---

## 멀티플레이어 지원

솔도로스는 멀티플레이어 전용 카드를 포함합니다.  
아군과 함께 플레이할 때만 등장하는 카드로 협력 시너지를 노려보세요.
